...
The node half snapshots each client bundle and available source map before publication. It groups resources into `/plugins/??...&rev=...` combo URLs, with one bootstrap combo for the modules row and one or more application combos for the other rows. Each phase is partitioned before a URL exceeds 3 KiB or a served body exceeds 1 MiB: a combo script is all-or-nothing, because the batch is one `<script>` whose first incomplete statement stops every later registration in the same file. The body limit yields where the URL limit does not — a single bundle above it is still addressable, so it forms its own batch instead of failing composition. Every combo map is Indexed Source Map v3 and uses an authored section when available or an identity section for the packaged bundle. Initial per-plugin revisions use process nonces, so startup does not hash every plugin; HMR hashes only an artifact reported as changed. Advertised responses are immutable, and an unknown combination or revision returns 404.

### Package assets

`registerAssets(id, assets)` serves a package's static resources at `/plugins/<id>/assets/<path>`, so a package can keep binary resources out of its JavaScript bundle without owning a route: every carrier that answers `/plugins` answers these too. Assets match on pathname alone, so a caller may append a version query for cache separation and any value still reaches the same bytes — assets are versioned by their owning dependency rather than by a content revision, and a stale cache key must not become a 404. Registration returns the disposer for exactly the paths it added, rejects a path that is already registered, and rolls back its own partial application on that rejection. Asset lifetime belongs to the registrant's effect, not to the graph: recomposition replaces the bundle response tables and leaves assets untouched.
